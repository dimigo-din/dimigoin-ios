    ...
    var body: some View {
        ZStack {
            Image(isLoggedIn ? "logo" : "dangermark")
                .templateImage(width: 60, isLoggedIn == false ? Color("red") : .accent)
                .opacity(isLoggedIn == false ? 0.4 : 0.25)
            GeometryReader { geometry in
                Rectangle().fill(isLoggedIn == false ? Color("red") : .accent).frame(width: geometry.size.width, height: 4)
            }
            if isLoggedIn {
                GeometryReader { geometry in
                    VStack {
                                    ...
                                    VStack {
                                        ...
                                        ForEach(loadTimetable(day: day-1), id: \.self) { lecture in
                                            Text("\(lecture)")
                                                ...
                                                .foregroundColor(getTodayDayOfWeekInt() == day ? .accent : .gray4)
                                        }
                                    }
                                    .padding(.vertical, 5)
                                }
