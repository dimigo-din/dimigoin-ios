...
struct NextMealWidget: View {
    ...
    var body: some View {
        ZStack {
            Image(data.tokenExist == false ? "dangermark" : "logo").renderingMode(.template).resizable().aspectRatio(contentMode: .fit).frame(width: 60).opacity(0.25).foregroundColor(data.tokenExist == false ? Color("red") : Color.accent)
            GeometryReader { geometry in
                Rectangle().fill(data.tokenExist == false ? Color("red") : Color.accent).frame(width: 4, height: geometry.size.height)
            }
